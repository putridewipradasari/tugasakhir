...
class Magangdt extends CI_Controller
{
	...
	function index()
	{
		if ($this->session->userdata('userdata')->role == 'Admin') {
			...
			$detail = $this->Magangm->get_magang_by_id($id);
			$this->load->view('magangdtv', $detail);
		} else {
	...
	public function download()
	{
		...
		force_download('assets/uploads/id-card/magang/idcard-' . $nik . '.png', NULL);
	...
	public function aksi_upload()
	{
		...
		$name = 'foto-' . $nik;
		$file = 'assets/uploads/foto/magang/foto-' . $nik . '.png';
		if ($file != NULL) {
			unlink('assets/uploads/foto/magang/foto-' . $nik . '.png');
			$config['file_name']        	= $name;
			$config['upload_path']          = 'assets/uploads/foto/magang/';
			...
			if (!$this->upload->do_upload('berkas')) {
				...
				redirect(base_url("Magangdt?id=" . $id));
			} else {
	...
	public function card()
	{
		...
		$result->writeFile('assets/uploads/qr-code/magang/item-' . $nik . '.png');
		// Save it to a file
		echo json_encode($result);

		$nik = $this->input->post('no_induk');
		$nama = $this->input->post('nama');
		$qr = 'assets/uploads/qr-code/magang/item-' . $nik . '.png';
		$foto = 'assets/uploads/foto/magang/foto-' . $nik . '.png';
		// template
		...
		$newWidth = 200;
		$newHeight = 200;
		$direktori = 'assets/uploads/resizeqr/magang/' . $nik . '.png';
		...
		$qrOk = imagecreatefrompng('assets/uploads/resizeqr/magang/' . $nik . '.png');
		...
		$newWidth1 = 500;
		$newHeight1 = 600;
		$direktori1 = 'assets/uploads/resizeft/magang/ft-' . $nik . '.png';
		...
		$fotoOk = imagecreatefrompng('assets/uploads/resizeft/magang/ft-' . $nik . '.png');

		// imagecopy($image, $qrOk, 140, 290, 0, 0, 200, 200);
		imagecopy($image, $fotoOk, 250, 450, 0, 0, 500, 600);

		// generate
		imagepng($image, 'assets/uploads/id-card/magang/idcard-' . $nik . '.png');
	}
}
